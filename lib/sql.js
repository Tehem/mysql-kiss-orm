 ...
 * @param {Number} [limit] the limit to apply if any
 * @param {Number} [offset] the offset to apply if any
...
/**
 * Buils a SELECT COUNT(*) SQL statement
 ...
 * @param {String} tableName the table name
 * @param {Object} match the delete matching restrictions (WHERE)
 * @param {Object} [options] optional parameters
 * @param {Object} [options.sort] optional sorting for results
 * @param {Number} [options.limit] optional limit for results
 * @return {string} the SQL statement
 */
function buildDeleteSql(tableName, match, options = { sort: {}, limit: 0 }) {
  return (
