...
module.exports = class MysqlConnector extends EventEmitter {
   ...
   *
   * @param {String} tableName name of the table in DB
   * @param {Object} match the delete restrictions (WHERE)
   * @param {Object} options delete options
   * @param {Object} [options.sort] optional sorting for row
   * delete (useful if limit is set)
   * @param {Number} [options.limit] optional row count limit
   * for delete
   * @return {Object} delete result object
   */
  async deleteMany(tableName, match, options = { sort: {}, limit: 0 }) {
    if (_.isEmpty(match)) {
      throw new Error('Invalid or empty match object for delete');
    }

    const sql = sqlLib.buildDeleteSql(tableName, match, options);
    const placeholders = Object.values(match);
    const [results] = await this.query(sql, placeholders);
    return results;
  }

  /**
   * Delete one document (the first matching) in table
   *
   * @param {String} tableName name of the table in DB
   * @param {Object} match the delete restrictions (WHERE)
   * @param {Object} options delete options
   * @param {Object} [options.sort] optional sorting for row delete
   * @return {Object} delete result object
   */
  deleteOne(tableName, match, options = { sort: {} }) {
    if (_.isEmpty(match)) {
      throw new Error('Empty matching object for single delete');
    }
